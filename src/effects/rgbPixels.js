...
var exec = function(main, params, callback){
	...
	for(var y = 0; y < main.canvas.HEIGHT - params['separator']; y += main.strokeResolution+params['separator']){
		for(var x = 0; x < main.canvas.WIDTH; x += main.strokeResolution){
			...
			main.buffer.ctx.fillRect(Math.ceil(x),Math.round(main.img.y+y),Math.ceil(main.strokeResolution/3), main.strokeResolution-1);
			
			main.buffer.ctx.fillStyle = 'rgba(0,'+main.avg[i][1]+',0,1)';
			main.buffer.ctx.fillRect(Math.ceil(x+main.strokeResolution/3),Math.round(main.img.y+y),Math.ceil(main.strokeResolution/3), main.strokeResolution-1);
			
			main.buffer.ctx.fillStyle = 'rgba(0,0,'+main.avg[i][2]+',1)';
			main.buffer.ctx.fillRect(Math.ceil(x+2*main.strokeResolution/3),Math.round(main.img.y+y),Math.ceil(main.strokeResolution/3), main.strokeResolution-1);
			i++;
		}
	}
