var exec = function(main){
	...
	for(var y = 0; y < main.img.i.height; y += main.strokeResolution){
		for(var x = 0; x < main.img.i.width; x += main.strokeResolution){
			...
			main.canvas.ctx.strokeRect(Math.round(main.canvas.WIDTH/2+x+main.innerMargin),Math.round(main.img.y+y),main.strokeResolution*(255-auxAvg)/255, main.strokeResolution*(255-auxAvg)/255);
			if(main.strokeResolution*(255-auxAvg)/255 > main.strokeResolution*0.8)
				main.canvas.ctx.strokeRect(Math.round(main.canvas.WIDTH/2+x+main.innerMargin+main.strokeResolution/3),Math.round(main.img.y+y+main.strokeResolution/3),main.strokeResolution/3, main.strokeResolution/3);
			i++;
		}
	}
}